...
export default function KretanjeProizvodaAdmin() {
  ...
  const workers = useMemo(() => {
    ...
    for (const row of rawData) {
      if (row.sifra_radnika > 0) set.add(row.sifra_radnika);
    }
  ...
  const confirmAiPrompt = () => {
    if (!selectedProductForAi) return;

    alert(
      `AI analiza je pokrenuta za proizvod: ${selectedProductForAi.naziv_proizvoda} (${selectedProductForAi.sifra_proizvoda})`,
    );
    closeAiPrompt();
  };
  ...
  return (
    <div className="pb-6">
      ...
      {visibleGroupedByPeriod.length === 0 ? (
        <div className="bg-gray-50 border border-gray-200 rounded-lg p-8 text-center">
          <p className="text-gray-600">
            Nema podataka za odabrani prikaz/radnika.
          </p>
        </div>
      ) : (
      ...
      {selectedProductForAi && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 p-3">
          <div className="w-full max-w-md rounded-xl border border-gray-200 bg-white p-4 shadow-2xl">
            <h3 className="text-sm font-semibold text-gray-900">
              AI analiza proizvoda
            </h3>
            <p className="mt-2 text-sm text-gray-700">
              Izabrali ste proizvod{" "}
              <strong>{selectedProductForAi.naziv_proizvoda}</strong> ({" "}
              {selectedProductForAi.sifra_proizvoda}) za AI analizu. Da li
              zelite da nastavite?
            </p>
